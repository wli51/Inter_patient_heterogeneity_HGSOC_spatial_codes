...
# this script performs cluster based DE expression analysis on malignant-rich clusters only
...
for (c in mal_rich_cluster_idx) {
  c_barcodes = rownames(obj.integrated@meta.data)[obj.integrated@meta.data$seurat_clusters == c]
  others_barcodes = rownames(obj.integrated@meta.data)[obj.integrated@meta.data$seurat_clusters %in% mal_rich_cluster_idx[!mal_rich_cluster_idx %in% c]]
  
  DE_df = FindMarkers( # comparing each malignant-rich cluster to all other malignant-rich clusters 
...
saveRDS(DE_df_list,
        file = file.path("intermediate_output", "integrate_mal_rich_cluster_DE.rds"))

DE_df_list = list()

for (c in mal_rich_cluster_idx) {
  c_barcodes = rownames(obj.integrated@meta.data)[obj.integrated@meta.data$seurat_clusters == c]
  others_barcodes = rownames(obj.integrated@meta.data)[obj.integrated@meta.data$seurat_clusters %in% c(fib_rich_cluster_idx, macro_rich_cluster_idx)]
  
  DE_df = FindMarkers( # comparing each malignant-rich cluster to all  non-malignant-rich clusters 
    obj.integrated@assays$integrated,
    assay = "integrated",
    cells.1 = c_barcodes,
    cells.2 = others_barcodes,
...
saveRDS(
  ...
  file = file.path(
    "intermediate_output",
    "integrate_mal_vs_other_cluster_DE.rds"
  )
)